(function (window, document) {
  ...
  var SUBJECTS = [
    { code: 'SAM3', name: 'Makroøkonomi', accent: '#ef4444', icon: '↗', status: 'active', summary: 'V25-pakken ligger ute med original eksamen, A-besvarelse og sensorveiledning.' },
    { code: 'RET14', name: 'Skatterett', accent: '#2f62ff', icon: '%', status: 'empty', summary: 'Ingen publiserte A-besvarelser er koblet på ennå.' },
    { code: 'SOL1', name: 'Organisasjonsatferd', accent: '#20b97a', icon: '♣', status: 'empty', summary: 'Ingen publiserte A-besvarelser er koblet på ennå.' },
    { code: 'SAM2', name: 'Mikroøkonomi', accent: '#f09828', icon: '◔', status: 'empty', summary: 'Ingen publiserte A-besvarelser er koblet på ennå.' }
  ];

  var PACKAGES = [
    {
      id: 'sam3-v25', subject: 'SAM3', term: 'V25', title: 'Våren 2025', subtitle: 'SAM3 Makroøkonomi',
      description: 'Komplett eksamenspakke: originaloppgave, A-besvarelse og sensorveiledning.',
      resources: [
        { id: 'sam3-v25-answer', kind: 'A-besvarelse', title: 'A-besvarelse SAM3 V25', subtitle: 'Makroøkonomi', desc: 'Eksempel på sterk besvarelse. Bruk den etter egen gjennomføring for å sammenligne struktur, modellbruk og drøfting.', icon: 'A', url: 'https://drive.google.com/file/d/1yCI-f4BKMTllsLc5ZMgh6pj0TIA428x5/view', download: 'https://drive.google.com/uc?export=download&id=1yCI-f4BKMTllsLc5ZMgh6pj0TIA428x5' },
        { id: 'sam3-v25-exam', kind: 'Eksamen', title: 'SAM3 skoleeksamen V25', subtitle: 'Original oppgave', desc: 'Original eksamensoppgave for våren 2025. Løs denne først før du åpner besvarelsen.', icon: 'E', url: 'https://drive.google.com/file/d/1VKZwcmQF9zGlR2Hwtjy0UnKWlhHEf7_5/view', download: 'https://drive.google.com/uc?export=download&id=1VKZwcmQF9zGlR2Hwtjy0UnKWlhHEf7_5' },
        { id: 'sam3-v25-sensor', kind: 'Sensorveiledning', title: 'SAM3 sensorveiledning V25', subtitle: 'Vurderingspunkter', desc: 'Sensorveiledningen viser hva sensor belønner og hvilke momenter som bør være med.', icon: 'S', url: 'https://drive.google.com/file/d/1myk7l12OsR-jZ76am6e-W7iS1u6FNuTy/view', download: 'https://drive.google.com/uc?export=download&id=1myk7l12OsR-jZ76am6e-W7iS1u6FNuTy' }
      ]
    }
  ];
  ...
  function allResources() { return PACKAGES.reduce(function (out, p) { return out.concat(p.resources.map(function (r) { return Object.assign({ subject: p.subject, term: p.term, packageId: p.id, packageTitle: p.title }, r); })); }, []); }

  function syncLegacyGlobals() {
    window.answers = allResources().map(function (r, index) {
      var subject = subjectByCode(r.subject) || SUBJECTS[0];
      return {
        course: r.subject, icon: subject.icon, color: subject.accent, term: r.term,
        title: r.title, subtitle: r.subtitle, type: r.kind.toLowerCase(), desc: r.desc,
        meta: [r.kind, 'PDF', r.term], popular: index + 1, url: r.url, download: r.download
      };
    });
  }
  ...
  function injectStyles() {
    ...
    style.textContent = [
      ...
      '.hf-subject-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px}',
      ...
      '.hf-subject-tile{min-height:210px;padding:18px;display:flex;flex-direction:column;gap:13px;cursor:pointer;text-align:left;color:#fff}',
      ...
      '.hf-subject-tile[disabled]{cursor:not-allowed;opacity:.72}',
      ...
      '.hf-resource-card{padding:17px;display:flex;flex-direction:column;gap:12px;min-height:235px}',
      ...
      '.hf-empty-panel{padding:28px;border-radius:22px;background:rgba(255,255,255,.035);border:1px dashed rgba(126,162,255,.25);color:#aebddd;line-height:1.6}',
      ...
      '.hf-flow-item .rank{width:28px;height:28px}',
      '.hf-flow-item strong{display:block;font-size:12.5px;color:#fff}.hf-flow-item span{display:block;color:#8fa5ca;font-size:11px;margin-top:2px}',
      '@media(max-width:1320px){.hf-subject-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.hf-resource-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}',
  ...
  function updateHero() {
    ...
    if (heroCopy) heroCopy.textContent = 'Arkivet er ryddet etter fag og semester, slik at siden bare viser dokumenter som faktisk er publisert. Start med fag, åpne en pakke og velg eksamen, A-besvarelse eller sensorveiledning.';
    if (stats[0]) stats[0].textContent = allResources().length;
    if (stats[1]) stats[1].textContent = PACKAGES.length;
    if (stats[2]) stats[2].textContent = '1';
    if (statLabels[0]) statLabels[0].textContent = 'PDF-er ute';
    if (statLabels[1]) statLabels[1].textContent = 'pakke';
    if (statLabels[2]) statLabels[2].textContent = 'aktivt fag';
  }
  ...
  function breadcrumb() {
    ...
    if (state.packageId) {
      ...
      html += '<span>›</span><span>' + esc(pack ? pack.title : 'Pakke') + '</span>';
    }
    html += '</div>';
    return html;
  }
  ...
  function renderSubjects() {
    var activeCount = PACKAGES.length;
    return breadcrumb() + '<div class="hf-answer-toolbar"><input class="hf-answer-search" id="hfAnswerSearch" type="search" placeholder="Søk i publiserte dokumenter..." value="' + esc(state.query) + '"><span class="hf-answer-muted">' + activeCount + ' publisert pakke · ' + allResources().length + ' PDF-er</span></div>'
      + '<div class="hf-subject-grid">' + SUBJECTS.map(function (subject) {
        var packs = packagesForSubject(subject.code);
        var count = packs.reduce(function (sum, p) { return sum + p.resources.length; }, 0);
        var live = count > 0;
        return '<button class="hf-subject-tile" style="--accent:' + subject.accent + '" data-subject="' + subject.code + '" ' + (live ? '' : 'disabled') + '>'
          + '<div class="hf-tile-top"><span class="hf-tile-icon">' + subject.icon + '</span><span class="hf-status-pill ' + (live ? 'live' : '') + '">' + (live ? 'Publisert' : 'Ingen filer') + '</span></div>'
          + '<h3>' + subject.code + '<br>' + subject.name + '</h3><p>' + subject.summary + '</p>'
          + '<div class="hf-tile-meta"><span class="hf-meta">' + packs.length + ' pakker</span><span class="hf-meta">' + count + ' PDF-er</span></div></button>';
      }).join('') + '</div>';
  }
  ...
  function renderSubject() {
    ...
    return breadcrumb() + '<div class="hf-answer-toolbar"><div><strong style="color:#fff">' + subject.code + ' · ' + subject.name + '</strong><div class="hf-answer-muted">Velg semester/eksamen før du åpner filer.</div></div><button class="hf-secondary" type="button" data-route="home">Alle fag</button></div>'
      + '<div class="hf-package-list">' + packs.map(function (p) {
  ...
  function renderPackage() {
    ...
    var resources = pack.resources.filter(function (r) {
      var q = state.query.toLowerCase().trim();
      if (!q) return true;
      return (r.title + ' ' + r.kind + ' ' + r.desc + ' ' + r.subtitle).toLowerCase().indexOf(q) !== -1;
    });
    return breadcrumb() + '<div class="hf-answer-toolbar"><input class="hf-answer-search" id="hfAnswerSearch" type="search" placeholder="Søk i ' + esc(pack.title) + '..." value="' + esc(state.query) + '"><button class="hf-secondary" type="button" data-route="subject" data-subject="' + pack.subject + '">Til ' + pack.subject + '</button></div>'
      + '<div class="hf-resource-grid">' + resources.map(function (r) {
  ...
  function renderSide() {
    ...
    if (!side) return;
    side.innerHTML = '<section class="side-panel"><div class="side-head"><h2>Dokumenter ute</h2></div><div class="popular-list">'
      + allResources().map(function (r, i) { return '<a class="popular-item" href="#/' + r.subject.toLowerCase() + '/' + r.packageId.replace(r.subject.toLowerCase() + '-', '') + '"><span class="rank">' + (i + 1) + '</span><div><strong>' + r.subject + ' ' + r.term + '</strong><span>' + esc(r.title) + '</span></div><span class="pop-tag">PDF</span></a>'; }).join('')
      + '</div></section><section class="side-panel"><div class="side-head"><h2>Anbefalt flyt</h2></div><div class="hf-flow"><div class="hf-flow-item"><span class="rank">1</span><div><strong>Løs eksamen først</strong><span>Ikke åpne A-besvarelsen før du har forsøkt selv.</span></div></div><div class="hf-flow-item"><span class="rank">2</span><div><strong>Sammenlign struktur</strong><span>Se disposisjon, modellbruk og drøftingsspråk.</span></div></div><div class="hf-flow-item"><span class="rank">3</span><div><strong>Les sensorveiledning</strong><span>Bruk vurderingspunktene til å finne neste øvingsmål.</span></div></div></div></section>';
  }

  function render() {
    syncLegacyGlobals();
    updateHero();
    var host = document.querySelector('.workspace > section');
    if (!host) return;
    host.innerHTML = '<div class="hf-answer-shell">' + (state.packageId ? renderPackage() : state.subject ? renderSubject() : renderSubjects()) + '</div><div class="empty" id="emptyState"></div>';
    renderSide();
    bind();
  }
  ...
  function bind() {
    document.querySelectorAll('[data-subject]').forEach(function (el) {
      ...
      el.addEventListener('click', function () { setHash(el.getAttribute('data-subject')); });
    });
    document.querySelectorAll('[data-package]').forEach(function (el) {
      el.addEventListener('click', function () { setHash(state.subject, el.getAttribute('data-package')); });
    });
