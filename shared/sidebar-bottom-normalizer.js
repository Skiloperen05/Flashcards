(function (window, document) {
  ...
  function name() {
    ...
    var session =
      window.AuthGuard && typeof window.AuthGuard.getSession === 'function' ? window.AuthGuard.getSession() : null;
  ...
  function injectStyles() {
    ...
    style.textContent = [
      ...
      '@media(max-width:1180px){.app>.sidebar:not(.hf-global-sidebar){display:none!important}}',
  ...
  function html() {
    ...
    return (
      '<div class="side-bottom" data-unified-bottom="1">' +
      '<div class="hf-week-card"></div>' +
      '<a class="hf-user-card user-mini" href="' +
      rootRelative('user/settings.html') +
      '"><span class="hf-avatar">' +
      esc(initial) +
      '</span><span><strong>' +
      esc(n) +
      '</strong><span>NHH-student</span></span><span class="arrow">›</span></a>' +
      '<a class="hf-premium-card" href="' +
      rootRelative('user/settings.html#premium') +
      '"><span class="crown">♛</span><strong>Premium</strong><span>Få tilgang til eksklusive verktøy og analyser.</span><b>Oppgrader nå →</b></a>' +
      '<a class="hf-exit-card" href="' +
      rootRelative('login.html') +
      '">Logg ut</a>' +
      '</div>'
    );
  }
